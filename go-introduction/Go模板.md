...
将上面的`main.go`文件编译执行，然后使用浏览器访问`http://127.0.0.1:8080`就能看到页面上显示了“Hello World”。 这就是一个最简单的模板渲染的示例，Go语言模板引擎详细用法请往下阅读。
...
func sayHello(w http.ResponseWriter, r *http.Request) {
	...
	// 自定义一个问候的模板函数
	hello := func(arg string) (string, error) {
    return "hello," + arg, nil
	}
	// 采用链式操作在Parse之前调用Funcs添加自定义的kua函数
	tmpl, err := template.New("hello").Funcs(template.FuncMap{"hello": hello}).Parse(string(htmlByte))
	if err != nil {
		fmt.Println("create template failed, err:", err)
		return
	}
...
我们可以在模板文件`hello.tmpl`中按照如下方式使用我们自定义的`hello`函数了。
