...
跟 C 语言家族中的所有语言一样，go 也全部是通过值来传递。这是指，一个函数永远得到的是其收到的参数的拷贝，就像有赋值语句把传过来的值赋值给了函数的参数。
...
函数定义时可以给返回值命名，这种方式跟在参数中定义变量是一样的。因此可以在函数体中直接使用这些变量，最后通过单一的`return`关键字返回。
...
- defer 函数的参数在函数移动到 defer 语句（但是不会马上执行)时获取
...
- 先入后出原则
- defer 可以读取并且修改函数的命名返回值

  下面的函数返回值是 2 而不是 1，fuc c() (i int){} 中的 i 就是命名返回值，跟声明一个参数没有区别。
...
Go 语言中目前（Go1.12）是没有异常机制，但是使用`panic/recover`模式来处理错误。 `panic`可以在任何地方引发，但`recover`只有在`defer`调用的函数中有效。如果在语句中执行 recover 函数将得到一个 nil 的返回值。首先来看一个例子：
